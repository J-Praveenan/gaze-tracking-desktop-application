...
# Load a small text-generation model (first run will download it)
predictor = pipeline("text-generation", model="distilgpt2", framework="pt")

# download dictionary (only first run)
try:
...
class VirtualKeyboard:
    ...
    def __init__(self, master=Tk()):
        ...
        width = self.size_value_map[self.size_current][0]
        height = self.size_value_map[self.size_current][1] + 120  # extra height for suggestion bar
        ...
        self.suggestion_frame = Frame(self.master, bg="lightgray")
        self.suggestion_frame.grid(row=0, column=0, columnspan=15, sticky="ew", pady=5)

        self.suggestion_buttons = []
        for i in range(5):  # up to 5 suggestions
            btn = Button(self.suggestion_frame, text="", state=DISABLED,
                        font=self.bottomfont,
                        bg="#f0f0f0", relief=RAISED,
                        command=lambda i=i: self.choose_suggestion(i))
            btn.grid(row=0, column=i, padx=5, pady=3, sticky="ew")
            self.suggestion_buttons.append(btn)

        self.current_word = ""  # buffer for typed word
        ...
        Label(infoframe7, text="Gaze Tracker...\nVirtual Keyboard", bg=self.gray, fg="white", font=self.neetfont).grid(row=0, column=10, sticky="NSEW")
    ...
    def update_suggestions(self):
        """Update suggestion bar based on AI model"""
        ...
        try:
            # Generate a few continuations
            outputs = predictor(
                self.current_word,
                max_new_tokens=5,        # only generate a few new tokens
                num_return_sequences=3,
                truncation=True,         # explicitly enable truncation
                pad_token_id=50256       # stop the warning about pad_token
            )


            # Extract the *next word* from each suggestion
            matches = []
            for o in outputs:
                text = o["generated_text"].strip()
                parts = text.split()
                if len(parts) > 1:
                    next_word = parts[1] if parts[0].lower() == self.current_word.lower() else parts[-1]
                    if next_word not in matches:
        ...
        for i, btn in enumerate(self.suggestion_buttons):
            if i < len(matches):
                btn.config(text=matches[i], state=NORMAL)
            else:
                btn.config(text="", state=DISABLED)


    def choose_suggestion(self, i):
        """Replace current prefix with chosen word"""
        word = self.suggestion_buttons[i].cget("text")
        if word:
            # delete typed prefix
            ...
            self.current_word = ""  # reset buffer
    ...
    def vpresskey(self, x):
        # Track word buffer
        if x == "spacebar" or x == "enter":
            self.current_word = ""
        elif x == "backspace":
            self.current_word = self.current_word[:-1]
        elif len(x) == 1 and x.isalpha():  # normal letters
            self.current_word += x
        else:
            self.current_word = ""
        ...
        # Keep existing press functionality
        self.master.unbind("<Unmap>", self.unmap_bind)
        self.master.withdraw()
        self.master.after(80, keyboard.send(x))
        self.master.after(10, self.master.wm_deiconify)
    ...
    def removekbfromtop(self):
        ...
        self.pinkb_button.config(bg="#5D7BEA", activebackground=self.darkblue, relief=RAISED, text="Pin Keyboard 📌")
        self.master.update()

    # enable the option to keep keyboard on top
    def addkbtotop(self):
        self.master.attributes('-topmost', True)
        self.pinkb_button.config(relief=SUNKEN, bg="#5D7BEA", activebackground=self.blue, text="Unpin Keyboard 📌")
        self.master.update()
