            ...
            def rest_reminder_timer():
                ...
                while not stop_reminder_event.is_set():
                    ...
                    if elapsed >= reminder_minutes * 60:
                        ...
                        toast.show()
                        speak_action_confirmation(f"You have been using Look Track Vision for {reminder_minutes} minutes. Take a short rest!")
...
class HomePage(BasePage):
    ...
    def _build_home_content(self, parent):
        ...
        # Initialize app_running early so it can be used in System Status
        ...
        control_card.grid(row=0, column=0, sticky="nsew", padx=8, pady=8)
        ...
        if reminder_path.exists():
            try:
                with open(reminder_path, "r") as f:
                    ...
                    reminder_duration = reminder_data.get("duration", 10)
            except Exception:
                pass

        status_text = (
            f"🟢 Gaze Control: {'Enabled' if self.app_running else 'Disabled'}\n"
            f"🔉 Voice Tips: {'Enabled' if common.voice_tips_enabled else 'Disabled'}\n"
            f"🗣️ Voice Confirmation: {'Enabled' if common.voice_action_confirmation else 'Disabled'}\n"
            f"⏰ Rest Reminder: {'ON (' + str(reminder_duration) + ' min)' if reminder_enabled else 'OFF'}"
        )

        status_card = RoundedCard(scroll_frame, radius=12, pad=12,
                                bg=Colors.glass_bg, border_color="#4b5563", border_width=2)
        status_card.pack(fill="x", padx=8, pady=(4, 10))

        tk.Label(status_card.body, text="System Status",
                fg=Colors.card_head, bg=Colors.glass_bg,
                font=F("h2b", ("Segoe UI", 12, "bold"))
                ).pack(anchor="w", padx=6, pady=(0, 4))

        tk.Label(status_card.body, text=status_text,
                fg=Colors.card_text, bg=Colors.glass_bg,
                font=F("body", ("Segoe UI", 10)), justify="left"
        ...
        def toggle_app():
            ...
            # When gaze starts — open floating tray
            if not self.app_running:
                tray = InstructionTray(self)
                tray.focus_force()
