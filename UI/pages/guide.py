...
# Expecting:
#   <project>/third_party/vlc/libvlc.dll
#   <project>/third_party/vlc/libvlccore.dll
#   <project>/third_party/vlc/plugins/...
...
if platform.system() == "Windows":
    if LIBVLC.exists():
        # Make DLLs discoverable by Windows loader
        try:
            os.add_dll_directory(str(VLC_DIR))
        except Exception:
            pass
        # Some python-vlc builds also look at PATH
        os.environ["PATH"] = str(VLC_DIR) + os.pathsep + os.environ.get("PATH", "")
        # Tell python-vlc exactly which DLL and plugin dir to use
        ...
        # Preload to surface clear error messages (e.g., 32/64-bit mismatch)
        try:
...
class GuideVideoPage(BasePage):
    def __init__(self, parent, controller, guide_video_path: Path):
        ...
        self.card.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.82, relheight=0.64)

        # Video container (surface for VLC)
        # inside __init__ (replace the old video_wrap)
        self.video = tk.Canvas(self.card.body, bg=self.card.cget("bg"), highlightthickness=0, bd=0)
        self.video.pack(fill="both", expand=True)
        self.video.bind("<Configure>", lambda e: self._attach_handle())  # keep handle fresh on resize


        # Controls row
        ctrl_bg = self.card.cget("bg")
        self.controls = tk.Frame(self.card.body, bg=ctrl_bg)
        self.controls.pack(fill="x", pady=(10, 0))

        left = tk.Frame(self.controls, bg=ctrl_bg); left.pack(side="left")
        ...
        mid = tk.Frame(self.controls, bg=ctrl_bg); mid.pack(side="left", fill="x", expand=True, padx=8)
        self.progress = tk.Canvas(mid, height=10, bg=ctrl_bg, highlightthickness=0, bd=0)
        ...
        right = tk.Frame(self.controls, bg=ctrl_bg); right.pack(side="right")
        self.btn_vol = tk.Button(right, text="🔊", command=self._toggle_mute,
                                 font=("Segoe UI Symbol", 11), fg="#ffffff",
                                 bg=ctrl_bg, bd=0, cursor="hand2")
        self.btn_vol.pack(side="left", padx=6)
        ...
        # Skip button
        self.skip = PillButton(self.overlay, text="SKIP  ⏭",
                               command=lambda: controller.show("HomePage"))
        self.skip.place(relx=0.92, rely=0.88, anchor="se")

        # Load the media if present
        if self.guide and Path(self.guide).exists():
            ...
            ph = tk.Label(self.video_wrap, text="( Put guide.mp4 in /assets )",
                          font=Fonts.h3, fg="#ffffff", bg=ctrl_bg)
            ph.pack(expand=True)

    # ----- VLC hooks -----
    def _attach_handle(self):
        """Bind VLC video output to our Canvas HWND (Windows)."""
        try:
            self.video.update_idletasks()
            hwnd = self.video.winfo_id()
            self.player.set_hwnd(hwnd)
            # optional: let VLC scale to fit
            self.player.video_set_scale(0)            # auto
            self.player.video_set_aspect_ratio(None)  # auto
        except Exception as e:
            print("set_hwnd failed:", e)

    def _poll_state(self):
        """Poll player state and update UI (duration/progress/end)."""
        try:
    ...
    def _toggle_play(self):
        ...
        if st in (vlc.State.Playing, vlc.State.Buffering):
            self.player.pause(); self.btn_play.config(text="▶")
        else:
            self.player.play();  self.btn_play.config(text="❚❚")
